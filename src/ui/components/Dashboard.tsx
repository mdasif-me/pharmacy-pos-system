...
export const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const {
    status: socketStatus,
    lastEvent: latestBroadcast,
    error: socketError,
  } = useSaleModeSocket(true)
  ...
  const formatCurrency = useCallback((value: number) => `BDT ${value.toFixed(2)}`, [])

  const formatSyncTimestamp = useCallback((timestamp: string) => {
    ...
    <div className="dashboard-root">
      ...
      <div className="dashboard-main">
        <header className="dashboard-header">
          ...
          <div className="dashboard-header-actions">
            <div className={`socket-indicator socket-${socketStatus}`}>
              <span className="socket-indicator-dot" />
              <span className="socket-indicator-label">{socketStatusLabel}</span>
            </div>
            <button
              type="button"
        ...
        <main className="dashboard-content">
          {(latestBroadcast || socketError) && (
            <section className="panel sale-mode-panel">
              <div className="panel-header">
                <h3>latest stock broadcast</h3>
                {latestBroadcast && (
                  <p>sync reference: {formatSyncTimestamp(latestBroadcast.sync_at)}</p>
                )}
                {socketError && <p className="sale-mode-error">{socketError}</p>}
              </div>
              {latestBroadcast && (
                <div className="sale-mode-body">
                  <div className="sale-mode-meta">
                    <span className="sale-mode-label">product id</span>
                    <span className="sale-mode-value">{latestBroadcast.product_id}</span>
                  </div>
                  <div className="sale-mode-meta">
                    <span className="sale-mode-label">in stock</span>
                    <span className="sale-mode-value">{latestBroadcast.in_stock}</span>
                  </div>
                  <div className="sale-mode-meta">
                    <span className="sale-mode-label">discount price</span>
                    <span className="sale-mode-value">
                      {formatCurrency(latestBroadcast.discount_price)}
                    </span>
                  </div>
                  <div className="sale-mode-meta">
                    <span className="sale-mode-label">peak-hour price</span>
                    <span className="sale-mode-value">
                      {formatCurrency(latestBroadcast.peak_hour_price)}
                    </span>
                  </div>
                  <div className="sale-mode-meta">
                    <span className="sale-mode-label">mediboy offer</span>
                    <span className="sale-mode-value">
                      {formatCurrency(latestBroadcast.mediboy_offer_price)}
                    </span>
                  </div>
                </div>
              )}
